...
- 时间选择：
  ...
  - 晚跑：17:30 开始（备选）
- 训练指标：
  - 前 15 分钟心率：区间 2（132-145 次/分钟）
  - 中间 30 分钟心率：区间 3-4（145-171 次/分钟）
  - 最后 15 分钟心率：区间 2（132-145 次/分钟）
  - 步频：178-180 步/分钟（中间段）
  - 配速：中间段 5:20-5:30/公里
- 训练流程：
...
- 训练流程：
  ...
  - 主要训练：
    ...
    - 10x400 米快跑（心率区间 4-5（158-186 次/分钟），配速 4:30/公里，约 1 分钟 48 秒/次）
    - 间隔：200 米慢跑恢复（心率区间 2（132-145 次/分钟），速度 9 公里/小时，约 1 分钟 20 秒/次）
    - 10 分钟慢跑结束（心率区间 2（132-145 次/分钟），速度 9.5 公里/小时）
  - 放松（12 分钟）：先步行 5 分钟（速度 4.5 公里/小时，坡度 0%）+ 7 分钟静态拉伸
- 训练指标：
  - 快跑心率：区间 4-5（158-186 次/分钟）
...
- 时间安排：
  ...
  - 尽量模拟比赛时间段进行
- 强度策略：
  - 保持区间3-4的心率（145-171 次/分钟），尽量模拟比赛强度
  - 在关键路段尝试冲刺强度（区间5，171-186 次/分钟）
- 技术要点：
  - 练习持续长时间的体力管理
  - 在疲劳状态下的决策和技术应对
- 补给计划：
